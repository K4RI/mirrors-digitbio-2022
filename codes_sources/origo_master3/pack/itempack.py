...
class Tree:
	def __init__(s, sets, cands, path, fr, target, K, treecache):
		s.trivial = False;
		s.leaf = True;
		s.neg = None;
		s.pos = None;
	...
	def best_tree(s):
		...
		for c in s.cands:
			newcands = s.cands - frozenset([c]);
			psets = split(c, s.sets)[0];

			ppath = (s.path[0] | frozenset([c]), s.path[1]);
			npath = (s.path[0], s.path[1] | frozenset([c]));
			treekey = (ppath, npath);

			if treekey in s.treecache:
				(pos, neg) = s.treecache[treekey];
			else:
				pos = Tree(psets, newcands, ppath, s.fr, s.target, s.K, s.treecache);
				neg = Tree(psets, newcands, npath, s.fr, s.target, s.K, s.treecache);
			
				pos.best_tree();
				neg.best_tree();

				s.treecache[treekey] = (pos, neg);
