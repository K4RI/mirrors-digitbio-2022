...
def init_diagram():
    L = nx.DiGraph()
    L.add_node(-1, inte=Pattern(dirty=False), exte=[], f=True)
    L.add_node(-2, inte=Pattern(dirty=False), exte=[], f=True)
    L.add_edge(-1, -2)
    return L
...
def clean_flags(L, concept_id):
    for j in L.neighbors(concept_id):
        if not L.nodes[j][flag]:
            clean_flags(L, j)
    L.nodes[concept_id][flag] = True
...
def get_maximal_concept(intent, generator_id, L):
    ...
    for j in L.neighbors(generator_id):
        # print('GENERATOR {} - SUPER {}'.format(generator_id,j))
        if intent == L.nodes[j][intent_mark]:  # THE CONCEPT ALREADY EXISTS. RETURN THE NODE ID OF THAT CONCEPT
            return j
        elif intent <= L.nodes[j][intent_mark]:  # THE MOST COMMON CASE
            return get_maximal_concept(intent, j, L)
    return generator_id


def add_object(object_concept_id, object_id, L, depth=1):
    if not L.nodes[object_concept_id][flag]:
        return 0
    #print("-"*depth,object,'->',intent,g.node[intent],'=',)
    L.nodes[object_concept_id][flag]=False
    L.nodes[object_concept_id][extent_mark].append(object_id)
    #print(g.node[intent])
...
def add_intent(intent, generator, L, tab, maximality, start_time):
    ...
    if generator != -1 and L.nodes[generator][intent_mark] == intent:
        return generator
    new_parents = []
    for candidate_id in L.neighbors(generator):
        if not L.nodes[candidate_id][intent_mark] <= intent:
            #print print_tab(tab), 'candidate_id', candidate_id
            candidate_intent = L.nodes[candidate_id][intent_mark].intersect(intent)
            candidate_id = add_intent(candidate_intent, candidate_id, L, tab + 1, maximality, start_time)
        add_parent = True
        for parent in new_parents:
            if L.nodes[candidate_id][intent_mark] <= L.nodes[parent][intent_mark]:
                add_parent = False
                break
            elif L.nodes[parent][intent_mark] <= L.nodes[candidate_id][intent_mark]:
    ...
    L.add_node(new_concept_id, exte=copy.copy(L.nodes[generator][extent_mark]), inte=intent, f=True)
    for parent in new_parents:
...
def check_maximality(child, parent, L):  # menghapus partition component yang tidak maksimal, tapi bermasalah (mungkin) buat similarity antar partitions
    child_intent_partition = set(L.nodes[child][intent_mark].partition)
    parent_intent_partition = L.nodes[parent][intent_mark].partition
    L.nodes[child][intent_mark].partition = set()
    for partition_component_c in child_intent_partition:
        ...
        if maximal:
            L.nodes[child][intent_mark].partition.add(partition_component_c)
    return
